...
namespace CarService
{
    public partial class ManagerForm : Form
    {
        ...
        {
            var result = MessageBox.Show("Вы действительно хотите выйти из раздела менеджера?", "Подтверждение",
                MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
            {
        ...
        private void buttonEditWork_Click(object sender, EventArgs e)
        {
            ...
            EditStatus form = new EditStatus((int)dgvWorks.SelectedRows[0].Cells["WorkPK"].Value, dgvWorks.SelectedRows[0].Cells["Status"].Value.ToString());
            form.ShowDialog();
            buttonWorks_Click(sender, e);
        }
        ...
        private void buttonTicket_Click(object sender, EventArgs e)
        {
            ...
            cell.Border = 0;
            table.AddCell(cell);
            cell.Phrase = new Phrase(tableCar.Rows[0].ItemArray[6].ToString(), new
                iTextSharp.text.Font(baseFont, iTextSharp.text.Font.DEFAULTSIZE, iTextSharp.text.Font.BOLD)
            { Size = 10 });
            cell.HorizontalAlignment = Element.ALIGN_LEFT;
            table.AddCell(cell);
